...
export async function getAllCurrencies(query) {
  ...
  return fetch(
    `${CG_API_URL}/coins/markets?vs_currency=${query.vs_currency}&order=${
      query.order || 'market_cap_desc'
    }&per_page=${query.perPage || 50}&page=${
      query.page || 1
...
export async function getMarketChartData(query) {
  const requestOptions = {
    method: 'GET',
    headers: { 'Content-Type': 'application/json' },
  };
  ...
  return fetch(
    `${CG_API_URL}/coins/${query.id}/market_chart?vs_currency=${query.vs_currency || 'usd'}&days=${
      query.days || 'max'
